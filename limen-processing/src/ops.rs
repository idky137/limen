...
use limen_core::routing::{JoinOperator, SplitOperator};
...
impl<const N: usize, T: Copy, const K: usize> SplitOperator<Tensor1D<T, N>, Tensor1D<T, N>, K>
    for CopySplit<Tensor1D<T, N>, K>
{
    fn split(
        &mut self,
        input: &Tensor1D<T, N>,
        _budget: Option<Ticks>,
        outputs: &mut [Tensor1D<T, N>; K],
    ) {
...
impl<const N: usize, const K: usize> SplitOperator<Tensor1D<f32, N>, Tensor1D<f32, { N / K }>, K>
    for ScatterSplit<N, K>
{
    fn split(
        &mut self,
        input: &Tensor1D<f32, N>,
        _budget: Option<Ticks>,
        outputs: &mut [Tensor1D<f32, { N / K }>; K],
    ) {
        let chunk = N / K;
        for i in 0..K {
...
impl<const N: usize, const K: usize> JoinOperator<Tensor1D<f32, N>, Tensor1D<f32, { N * K }>, K>
    for ConcatJoin<N, K>
{
    fn join(
        &mut self,
        inputs: &[Tensor1D<f32, N>; K],
        _budget: Option<Ticks>,
    ) -> Tensor1D<f32, { N * K }> {
        let mut out = [0.0f32; { N * K }];
        for i in 0..K {
...
impl<const N: usize, const K: usize> JoinOperator<Tensor1D<f32, N>, Tensor1D<f32, N>, K>
    for SumJoin<N, K>
{
